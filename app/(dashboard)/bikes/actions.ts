...
function bikePayload(formData: FormData) {
  ...
  //
  // Passer par `optional` plutôt que par `Number(formData.get(...))` : sur un
  // champ absent, `Number(null)` vaut `0` — le vélo naissait avec 0 % de décote
  // au lieu du défaut de sa catégorie.
  const tauxSaisi = optional("depreciation_rate");
  const taux = tauxSaisi !== null ? Number(tauxSaisi) : Number.NaN;
